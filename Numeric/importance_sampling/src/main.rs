...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    df.push(
        "Error",
        Series::new(vec![error_g7k15r, error_uniform, error_weighted]),
    );
    df.push("Value", Series::new(vec![g7k15r, uniform, weighted]));
...
fn importance_sampling_weighted<F>(f: F, (a, b): (f64, f64), n: usize) -> anyhow::Result<f64>
where
    F: Fn(f64) -> f64 + Copy,
{
    ...
    Ok(samples
        .into_iter()
        .fold(0f64, |acc, x| acc + f(x) / weighted.pdf(x))
        / n as f64)
}
